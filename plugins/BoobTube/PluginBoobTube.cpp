...
#include "BoobTube.cc"
#include "BoobTube60V.cc"

START_NAMESPACE_DISTRHO

// -----------------------------------------------------------------------

PluginBoobTube::PluginBoobTube()
...
void PluginBoobTube::initParameter(uint32_t index, Parameter& parameter) {
    if (index >= paramCount)
        return;
    ...
    switch (index) {
        ...
        case VOLUME:
            ...
            break;
        case BOOST:
            parameter.name = "40/60V";
            parameter.shortName = "40/60V";
...
*/
void PluginBoobTube::sampleRateChanged(double newSampleRate) {
...
*/
float PluginBoobTube::getParameterValue(uint32_t index) const {
...
*/
void PluginBoobTube::setParameterValue(uint32_t index, float value) {
    fParams[index] = value;
...
void PluginBoobTube::setOutputParameterValue(uint32_t index, float value)
{
    fParams[index] = value;
...
void PluginBoobTube::run(const float** inputs, float** outputs,
                              uint32_t frames) {
   ...
   // float* const outR = outputs[1];
    ...
    if (!bypassed) {
        ...
        if (!boost_is) dsp->compute(frames, buf, buf);
    }
