...
void PluginRumor::initParameter(uint32_t index, Parameter& parameter) {
    if (index >= paramCount)
        return;
...
*/
void PluginRumor::sampleRateChanged(double newSampleRate) {
...
*/
float PluginRumor::getParameterValue(uint32_t index) const {
...
*/
void PluginRumor::setParameterValue(uint32_t index, float value) {
    fParams[index] = value;
    //fprintf(stderr, "setParameterValue %i %f\n", index,value);
    dsp->connect(index, value);
}

void PluginRumor::setOutputParameterValue(uint32_t index, float value)
{
    fParams[index] = value;
...
void PluginRumor::run(const float** inputs, float** outputs,
                              uint32_t frames) {
